...
@section('content')
    ...
    <div class="main-content">
        <h1>Специальности колледжа</h1>
        <a href="{{ route('college_specialties.create') }}" class="btn btn-primary">Добавить специальность</a>
        ...
        <ul>
            @foreach($specialties as $specialty)
                <li class="event-card">
                    <span style="font-size: 22px">{{ $specialty->name }}</span>
                    <p>{{ $specialty->description }}</p>
                    <div class="button-group">
                        <a href="{{ route('college_specialties.edit', $specialty->id) }}">
                            <button class="btn btn-sm btn-warning">
                                <i class="fas fa-edit"></i> Редактировать
                            </button>
                        </a>
                        <form action="{{ route('college_specialties.destroy', $specialty->id) }}" method="POST" class="delete-form">
                            @csrf
                            @method('DELETE')
