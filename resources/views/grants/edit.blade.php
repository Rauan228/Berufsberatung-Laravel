...
<head>
    ...
    <title>Редактировать грант</title>
    <style>
        body {
...
<body>
    <div class="container">
        ...
        <form action="{{ route('grants.update', $grant->id) }}" method="POST">
            ...
            <input type="text" name="grant_name" id="grant_name" value="{{ $grant->grant_name }}" required>

            <label for="amount">Сумма</label>
            <input type="number" name="amount" id="amount" step="0.01" value="{{ $grant->amount }}" required>

            <label for="application_deadline">Срок подачи заявки</label>
            <input type="date" name="application_deadline" id="application_deadline" value="{{ $grant->application_deadline }}" required>
